...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/action_items/autocomplete`, { params });
  return response.data;
}

const Action_itemsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.action_items.hasPermissionToCreate,
});

export default connect(select)(Action_itemsSelectItem);
