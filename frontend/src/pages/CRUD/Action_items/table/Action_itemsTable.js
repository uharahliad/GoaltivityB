...
import * as dataFormat from 'pages/CRUD/Action_items/table/Action_itemsDataFormatters';

// eslint-disable-next-line
import * as goalsDataFormat from 'pages/CRUD/Goals/table/GoalsDataFormatters';

import actions from 'actions/action_items/action_itemsListActions';
...
const Action_itemsTable = () => {
  ...
  const count = useSelector((store) => store.action_items.list.count);
  const modalOpen = useSelector((store) => store.action_items.list.modalOpen);
  const rows = useSelector((store) => store.action_items.list.rows);
  const idToDelete = useSelector((store) => store.action_items.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Action_items')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/action_items/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/action_items/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Action_items'
                : 'http://localhost:8080/api-docs/#/Action_items'
            }
          >
            API documentation for action_items
          </LinkMaterial>
        </div>
      </Widget>
...
export default Action_itemsTable;
