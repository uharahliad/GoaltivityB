...
// eslint-disable-next-line
import * as accountability_groupsDataFormat from 'pages/CRUD/Accountability_groups/table/Accountability_groupsDataFormatters';
...
const MessagesTable = () => {
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        accountability_groupsDataFormat.listFormatter(
          params.row[params.field],
          history,
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Messages')}</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/messages/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
