...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/goals/autocomplete`, { params });
  return response.data;
}

const GoalsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.goals.hasPermissionToCreate,
});

export default connect(select)(GoalsSelectItem);
