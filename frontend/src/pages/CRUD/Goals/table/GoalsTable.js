...
import * as dataFormat from 'pages/CRUD/Goals/table/GoalsDataFormatters';

// eslint-disable-next-line
import * as goal_categoriesDataFormat from 'pages/CRUD/Goal_categories/table/Goal_categoriesDataFormatters';
// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';

import actions from 'actions/goals/goalsListActions';
...
const GoalsTable = () => {
  ...
  const count = useSelector((store) => store.goals.list.count);
  const modalOpen = useSelector((store) => store.goals.list.modalOpen);
  const rows = useSelector((store) => store.goals.list.rows);
  const idToDelete = useSelector((store) => store.goals.list.idToDelete);
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        goal_categoriesDataFormat.listFormatter(
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Goals')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/goals/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/goals/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Goals'
                : 'http://localhost:8080/api-docs/#/Goals'
            }
          >
            API documentation for goals
          </LinkMaterial>
        </div>
      </Widget>
