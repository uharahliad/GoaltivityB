...
import * as dataFormat from 'pages/CRUD/Goal_categories/table/Goal_categoriesDataFormatters';

import actions from 'actions/goal_categories/goal_categoriesListActions';
...
const Goal_categoriesTable = () => {
  ...
  const count = useSelector((store) => store.goal_categories.list.count);
  const modalOpen = useSelector(
    (store) => store.goal_categories.list.modalOpen,
  );
  const rows = useSelector((store) => store.goal_categories.list.rows);
  const idToDelete = useSelector(
    (store) => store.goal_categories.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Goal_categories')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/goal_categories/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/goal_categories/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Goal_categories'
                : 'http://localhost:8080/api-docs/#/Goal_categories'
            }
          >
            API documentation for goal_categories
          </LinkMaterial>
        </div>
      </Widget>
...
export default Goal_categoriesTable;
