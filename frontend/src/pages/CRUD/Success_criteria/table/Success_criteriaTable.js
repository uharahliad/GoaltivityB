...
import * as dataFormat from 'pages/CRUD/Success_criteria/table/Success_criteriaDataFormatters';

// eslint-disable-next-line
import * as goalsDataFormat from 'pages/CRUD/Goals/table/GoalsDataFormatters';

import actions from 'actions/success_criteria/success_criteriaListActions';
...
const Success_criteriaTable = () => {
  ...
  const count = useSelector((store) => store.success_criteria.list.count);
  const modalOpen = useSelector(
    (store) => store.success_criteria.list.modalOpen,
  );
  const rows = useSelector((store) => store.success_criteria.list.rows);
  const idToDelete = useSelector(
    (store) => store.success_criteria.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Success_criteria')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/success_criteria/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/success_criteria/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Success_criteria'
                : 'http://localhost:8080/api-docs/#/Success_criteria'
            }
          >
            API documentation for success_criteria
          </LinkMaterial>
        </div>
      </Widget>
...
export default Success_criteriaTable;
